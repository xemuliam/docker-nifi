...
do_site2site_configure() {
  sed -i "s/nifi\.remote\.input\.host=.*/nifi.remote.input.host=node-${MYID}/g" ${NIFI_HOME}/conf/nifi.properties
...
# NiFi properties
  sed -i "s/nifi\.web\.http\.host=.*/nifi.web.http.host=node-${MYID}/g" ${NIFI_HOME}/conf/nifi.properties
  ...
  sed -i "s/nifi\.cluster\.node\.address=.*/nifi.cluster.node.address=node-${MYID}/g" ${NIFI_HOME}/conf/nifi.properties
